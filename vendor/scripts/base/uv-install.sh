...
#
# uv Installation Script (Vendored with pinned version)
#
# SECURITY: This script is vendored locally with a fixed version (0.9.26) and
# hardcoded SHA256 checksums for each platform. The checksums were added to
# the platform case branches in download_binary_and_run_installer().
#
# To update uv version:
# 1. Update APP_VERSION below
# 2. Update ARTIFACT_DOWNLOAD_URL
# 3. Update checksums in each platform case branch
#    (get from https://github.com/astral-sh/uv/releases/download/$VERSION/*.sha256)
#
...
download_binary_and_run_installer() {
    ...
    case "$_artifact_name" in 
        "uv-aarch64-apple-darwin.tar.gz")
            ...
            _checksum_value="fcf0a9ea6599c6ae28a4c854ac6da76f2c889354d7c36ce136ef071f7ab9721f"
            ;;
        ...
        "uv-aarch64-unknown-linux-gnu.tar.gz")
            ...
            _updater_bin=""
            _checksum_style="sha256"
            _checksum_value="f71040c59798f79c44c08a7a1c1af7de95a8d334ea924b47b67ad6b9632be270"
            ;;
        "uv-aarch64-unknown-linux-musl.tar.gz")
            ...
            _updater_bin=""
            _checksum_style="sha256"
            _checksum_value="ba8698c36c00c22efed4bd3506339b03c95604d001f02eaf6fbc814c9224d801"
            ;;
        ...
        "uv-x86_64-apple-darwin.tar.gz")
            ...
            _updater_bin=""
            _checksum_style="sha256"
            _checksum_value="171eb8c518313e157c5b4cec7b4f743bc6bab1bd23e09b646679a02d096a047f"
            ;;
        ...
        "uv-x86_64-unknown-linux-gnu.tar.gz")
            ...
            _updater_bin=""
            _checksum_style="sha256"
            _checksum_value="30ccbf0a66dc8727a02b0e245c583ee970bdafecf3a443c1686e1b30ec4939e8"
            ;;
        "uv-x86_64-unknown-linux-musl.tar.gz")
            ...
            _updater_bin=""
            _checksum_style="sha256"
            _checksum_value="708b752876aeeb753257e1d55470569789e465684c1d3bc1760db26360b6c28b"
            ;;
        *)
...
downloader() {
    ...
    then need_cmd "$_dld"
        ...
        if [ -n "${AUTH_TOKEN:-}" ]; then
            # Use curl config file to avoid exposing token in process list
            _curl_config=$(mktemp)
            chmod 600 "$_curl_config"
            printf 'header = "Authorization: Bearer %s"\n' "$AUTH_TOKEN" > "$_curl_config"
            curl -sSfL --config "$_curl_config" "$1" -o "$2"
            _curl_exit=$?
            rm -f "$_curl_config"
            return $_curl_exit
        else
        ...
        if [ -n "${AUTH_TOKEN:-}" ]; then
            # Use wget header file to avoid exposing token in process list
            _wget_headers=$(mktemp)
            chmod 600 "$_wget_headers"
            printf 'Authorization: Bearer %s\n' "$AUTH_TOKEN" > "$_wget_headers"
            wget --header="$(cat "$_wget_headers")" "$1" -O "$2"
            _wget_exit=$?
            rm -f "$_wget_headers"
            return $_wget_exit
        else
